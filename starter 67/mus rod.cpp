...
template<typename T_container, typename T = typename enable_if<!is_same<T_container, string>::value, typename T_container::value_type>::type> ostream& operator<<(ostream &os, const T_container &vodka) { os << '{'; string sep; for (const T &x : vodka) os << sep << x, sep = ", "; return os << '}'; }
...
int main()
{
    ...
    while(t--)
    {
        ...
        vector<pair<ll,ll>>vodka;
        for(int i=0;i<n;i++)
        vodka.push_back({a[i],b[i]});
    sort(vodka.begin(),vodka.end(),comparator);
    ll i=0;
    ll answer=0;
    for(auto x:vodka){
        answer += x.second * i;
