...
int AYUSH_CHAURASIA(vector<ll>vl,ll n){
    ...
    while(i<n/2){
        ...
        ll ival = n - i - 1;if(vl[i] > vl[ival]){
            dopa[ival] = vl[i] - vl[ival];}
        else
...
// # 	}
        ...
        while(i<n && dopa[i] != 0) {
            if(i > 0 )
               { if(dopa[i] > dopa[i-1])
                ans = ans + dopa[i] - dopa[i-1];
            }

            // eguackahclaclacklclkasca
            // eguackahclaclacklclkasca
            // eguackahclaclacklclkasca
...
int main()
{
    ...
    while(t--)
    {
        ...
        vector<ll>vl(n);
        for(auto &x:vl)cin>>x;
        cout<<AYUSH_CHAURASIA(vl,n)<<endl;
    }
